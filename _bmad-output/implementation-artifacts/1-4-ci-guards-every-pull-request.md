...
- [x] [Review][Defer→Patch→Reverted] `apt-get install` for Tauri Linux system deps was unpinned and uncached [.github/workflows/ci.yml] — first **fixed** by replacing the raw `apt-get update && apt-get install` step with `awalsh128/cache-apt-pkgs-action@v1`, then **reverted** after it broke a real CI run: the action's first attempt errored out partway through populating its cache (never wrote the actual `.tar` package archives or `manifest_all.log`), and because GitHub Actions cache keys are immutable once saved, every subsequent run got a "cache hit" against that broken snapshot and restored zero packages — silently skipping `libwebkit2gtk-4.1-dev` and its transitive deps entirely (`glib-2.0`/`gio-2.0`/`gobject-2.0` pkg-config failures on `cargo check`). The action also bootstraps a second unpinned third-party tool (`apt-fast`, via `curl | bash` from `ilikenwf/apt-fast`) invisibly on cache miss. **Final resolution:** reverted to plain `apt-get`, matching Tauri's own official CI docs (verified via Context7) — a comment in `ci.yml` documents why the caching action was tried and abandoned. Uncached-but-reliable beats cached-but-fragile here.
