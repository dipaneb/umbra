...
- [x] **Task 1: `umbra-core::base64` — the functional core (AC: 1, 2, 3)**
  - [x] `crates/umbra-core/Cargo.toml`: add `base64 = "0.23"` (unpinned major, matching this project's existing convention for `tauri-plugin-store`/`tauri-plugin-clipboard-manager` — see Library/Framework Requirements below for exact API).
  - [x] New file `crates/umbra-core/src/base64.rs`:
  ...
  - [x] Reuse the `check_input_size`-style guard from `crates/umbra-core/src/json.rs` (same rationale: CWE-400 unbounded-allocation protection) — a pasted Base64 blob (e.g. an embedded font or image data URI) can be very large even without going through Story 2.2's file-drop path. Pick your own error code (e.g. `base64-input-too-large`) and reuse `json.rs`'s `MAX_INPUT_BYTES` constant value as a starting point, not a shared import (each tool module owns its own constant, per that file's existing pattern).
  - [x] Register the module in `crates/umbra-core/src/lib.rs`: add `pub mod base64;` alongside the existing `pub mod json;`.
  - [x] **Anti-pattern to avoid — this is the single most likely way this story quietly fails FR12:** a decoded Base64 payload can be arbitrary binary (e.g. a JWT signature segment, compressed data, an image). Naively converting decoded bytes to a displayable string via `String::from_utf8_lossy(&bytes)` *silently* substitutes U+FFFD replacement characters for invalid sequences instead of erroring — this satisfies neither "clear inline error" (AC3) nor "never... silent empty output" (FR12), because the user sees mangled-but-present text with no indication anything was wrong. Use `String::from_utf8(bytes)` (fallible) and map its `Err` to a `ToolError` with its own distinct `code` (e.g. `base64-not-utf8`) — do not conflate this with the alphabet-decode error's `code`, since they are different failure causes a user would want to distinguish.

- [x] **Task 2: Tauri commands — `base64_encode` / `base64_decode` (AC: 1, 2, 3)**
  - [x] New file `src-tauri/src/commands/base64.rs`, mirroring `src-tauri/src/commands/json.rs`'s exact shape:
  ...
  - [x] Register the module in `src-tauri/src/commands/mod.rs`: add `pub mod base64;` alongside `pub mod json;`.
  - [x] Register both commands in `src-tauri/src/lib.rs`'s `tauri::generate_handler![...]` list (currently `greet, json_format, json_minify, json_parse`) and the matching `use commands::base64::{base64_encode, base64_decode};` import line.

- [x] **Task 3: `Base64View.vue` — the tool's UI (AC: 1, 2, 3, 4)**
  - [x] New file `src/tools/base64/Base64View.vue`. Mirror `src/tools/json/JsonView.vue`'s established shape and conventions exactly — this story is the second tool ever built, and the first real test of whether that pattern generalizes:
...
- [x] **Task 4: Tests**
  ...
  - [x] `src-tauri/src/commands/base64.rs` command-level tests (mirror `json.rs`'s `#[tokio::test]` block): `base64_encode`/`base64_decode` happy paths for both alphabets, malformed-input error propagation, and a `map_join_error`-equivalent panic-mapping test.
  - [x] `src/tools/base64/Base64View.spec.ts`: Encode/Decode button actions call the right `invoke()` command with the right args (including `url_safe` for Encode); error rendering for both a byte-offset decode error and a generic error; paste/copy delegate to the mocked `clipboard.ts` functions (reuse the exact `vi.mock` module-factory pattern from `src/shell/clipboard.spec.ts` / `JsonView.spec.ts` — mock `@tauri-apps/api/core`'s `invoke` the same way `JsonView.spec.ts` does, do not invent a new mocking style).
  - [x] `src/stores/registry.spec.ts` (or wherever registry coverage lives — check for an existing spec before creating a new file) and/or `AppSidebar.spec.ts`/`CommandPalette.spec.ts`: extend existing "N tools registered" assertions to account for the new Base64 entry, and add a palette-search case confirming `"b64"` resolves to it (FR2).
...
- `cargo build` (src-tauri, dev profile) compiles cleanly with the new commands registered in `generate_handler!`, confirming the desktop target builds. Full interactive verification (encode/decode both alphabets, byte-offset error rendering, clipboard round-trip, sidebar/⌘K discovery) still needs a human at the native window — no CDP-equivalent bridge exists for a Tauri/WKWebView window from this agent's tools (only Chrome-tab browser automation is available).
- One frontend test-timing issue found and fixed during Task 4: `CommandPalette.spec.ts`'s Enter-to-navigate tests originally used a single `flushPromises()` after dispatching Enter, which was reliable against the old synthetic test component (`() => Promise.resolve({...})`, resolves in one microtask) but flaked against the Base64 entry's real `() => import("../tools/base64/Base64View.vue")` (a genuine dynamic import needing more than one microtask tick to resolve under Vite/vitest). Fixed by polling with `vi.waitFor(...)` instead of assuming a fixed number of flushes.

### Completion Notes List

- `crates/umbra-core/src/base64.rs`: custom `GeneralPurpose` decode engines (`DecodePaddingMode::Indifferent`) tolerate both padded and unpadded input, per the JWT-segment use case in the story's headline — the premade `STANDARD`/`URL_SAFE` constants would have rejected unpadded tokens. `decode` maps `DecodeError::InvalidByte`/`InvalidLastSymbol` to `Position::ByteOffset`, verified in tests to match the actual invalid byte's index (not just "some" offset).
- Followed the anti-pattern warning in Task 1 precisely: decoded bytes go through fallible `String::from_utf8` (not `from_utf8_lossy`), with a distinct `base64-not-utf8` code separate from `base64-invalid`, covered by a dedicated regression test using non-UTF-8-but-valid-Base64 input (`[0xff, 0xfe]`).
- `Base64View.vue` uses two explicit Encode/Decode buttons per the story's resolved interaction model (no auto-detection of user intent) — the alphabet radio only affects Encode; Decode has no alphabet control since auto-detection is handled entirely in the core `decode` function.
- Registered the `base64` entry as the second item in `registry.ts`'s `TOOLS` array (not inside `useRegistryStore`'s `ref()` call), per the post-#23 refactored shape; `assertUniqueToolIds` continues to pass at module load.
- `CommandPalette.spec.ts` previously pushed a synthetic `"b64"`-id tool on top of the real (JSON-only) registry to exercise multi-result behavior. With a real second entry now in the registry, that synthetic push would have created a duplicate "Base64"-named result and skewed rankings/counts, so it was removed in favor of exercising the real `base64` entry directly — including a new case asserting the `"b64"` alias itself resolves via the palette (FR2).
- All automated gates pass: `cargo fmt --check`, `cargo clippy -D warnings`, `cargo test --workspace` (53 tests, up from 32: +9 `umbra-core::base64`, +5 `commands::base64`, +7 net new palette/router assertions on the TS side), `pnpm lint`, `pnpm test` (99 tests, 16 files), `pnpm build`, `vue-tsc --noEmit`.
- **Outstanding:** Task 5's manual `pnpm tauri dev` walkthrough (encode/decode both alphabets, invalid-Base64 inline error with offset, clipboard paste/copy round-trip, sidebar + ⌘K discovery of "base64"/"b64") has not been performed interactively. Per this session's explicit user decision, the user will run this check themselves rather than have the agent launch a native window it cannot see or drive — flagged for hands-on verification before/during PR review.

### File List

- `crates/umbra-core/Cargo.toml` (+`base64 = "0.23"`)
- `Cargo.lock`
- `crates/umbra-core/src/lib.rs` (+`pub mod base64;`)
- `crates/umbra-core/src/base64.rs` (new)
- `src-tauri/src/commands/mod.rs` (+`pub mod base64;`)
- `src-tauri/src/commands/base64.rs` (new)
- `src-tauri/src/lib.rs` (+2 commands in `generate_handler!` + import)
- `src/tools/base64/Base64View.vue` (new)
- `src/tools/base64/Base64View.spec.ts` (new)
- `src/stores/registry.ts` (+1 tool entry)
- `src/router/index.spec.ts` (registry-length assertion updated 1 → 2)
- `src/shell/CommandPalette.spec.ts` (removed synthetic test entry, added real `base64`/`b64` cases, `vi.waitFor` timing fix)
