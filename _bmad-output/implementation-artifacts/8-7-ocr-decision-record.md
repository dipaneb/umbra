...
**Capture route — developer's decision: split.** The 8.6 precedent. The two cuts with real future weight are filed as individual max-context `backlog-candidate` GitHub issues on `dipaneb/umbra`, each linking back to this record; the rest live in this table and move to the developer's own tracking. **Filed to GitHub 2026-09-06:** cut #13 → [**#132**](https://github.com/dipaneb/umbra/issues/132) (OCR a scanned PDF), cut #14 → [**#133**](https://github.com/dipaneb/umbra/issues/133) (QR codes), cut #15 → [**#134**](https://github.com/dipaneb/umbra/issues/134) (hardware-accelerated inference) — each named explicitly by the developer, each carrying its full cost sheet so it is not re-argued. Everything else: personal backlog, logged here so nothing is silently dropped.
...
| **13** | **OCR a scanned PDF page — rasterize the page, then extract (PDF tier 3).** → [**#132**](https://github.com/dipaneb/umbra/issues/132) | The highest-value cut in this record: it is the one job the developer named that *nothing* serves. Cut on cost, not on value — the full cost sheet travels with the issue so nobody re-runs the argument. See *The PDF tiers* below. |
...
| **6** | **No execution provider configured**, so `ort` runs **CPU**. `OrtExecutionProvider::CoreML` exists in the config enum and is unused. | On Apple Silicon — the platform FR24's *"under ~3 s"* bar is explicitly written against — the neural engine is untouched. | **CUT → GitHub [#134](https://github.com/dipaneb/umbra/issues/134)**, with the provider analysis below. The developer's measured ~3 s baseline is acceptable *once there is feedback*, which makes acceleration an optimisation rather than a necessity. |
...
| **3** | Actually rasterize the page and run OCR on it. | **Cut → GitHub [#132](https://github.com/dipaneb/umbra/issues/132)**, with the cost sheet below attached. |
