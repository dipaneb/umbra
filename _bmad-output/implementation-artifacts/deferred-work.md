...
## Deferred from: code review of story-8-1-reimagine-the-json-formatter-viewer (2026-08-26)

- Client-side JS full-tree walks (Explorer's `findMatches` on every debounced keystroke, Diff's `flattenDiffTree` on every diff result) have no performance testing against the 10MB ceiling, unlike every new Rust command in this story. AC14's literal text scopes its performance sanity-check requirement to `spawn_blocking`-dispatched commands, so this is outside the AC's letter but arguably within its spirit. [`src/tools/json/flattenJsonTree.ts`, `src/tools/json/flattenDiffTree.ts`]
- `findMatches` (Explorer search) undercounts occurrences for a truncated (>80 char) leaf when some but not all matches sit past the truncation boundary. The code already has a documented fallback for the "all-hidden" case; this "partially-hidden" case silently drops the extra count. Narrow edge case, and arguably self-consistent with the code's own stated design (only count what's visibly markable). [`src/tools/json/flattenJsonTree.ts:230-235`]
- Query tab's own completion notes ("no live visual pass was possible," browser tooling unavailable) never get a follow-up "belated visual confirmation" entry the way Diff's did. A same-day entry about a copy-button hover bug suggests Query was exercised live at least once, so this is a process note, not a confirmed defect. Worth a quick manual double-check. [story completion notes, 2026-08-26]
- Format/Minify's top error banner and the Validate tab's own error can display the same underlying failure simultaneously. Already self-acknowledged in-code as "Provisional placement," a known non-blocking rough edge, not new information from this review. [`src/tools/json/JsonView.vue:477-483`]
- No fuzz/property-based testing for three new hand-written algorithms (`RepairScanner`, `diff_values`, `to_typescript`) — coverage is exclusively hand-picked example tests, the same style that missed two real bugs this same review found in `RepairScanner` (now patched). [`crates/umbra-core/src/json.rs`]
