...
16. **Given** `OcrOutcome { text: String, confidence: Option<f32> }` destroys, at the core boundary, per-region data the adapter already holds — `run_ocr` builds two parallel vectors, averages one and joins the other — **when** Task 2b completes, **then** `crates/umbra-core/src/ocr.rs` returns a region-structured outcome from **both** `OcrEngine` methods:
    ...
    pub struct OcrRegion {
        ...
        /// One quad per character of `text`, index-aligned with it by Unicode scalar, measured
        /// from the region's own ink. EMPTY when measurement is not confident — see the AC16
        /// amendment below. (Added at the character-measurement round; recorded here at code
        /// review, 2026-09-08, because this struct is the AC's normative statement of the shape.)
        pub char_polygons: Vec<Vec<OcrPoint>>,
    }
    pub struct OcrPoint { pub x: f32, pub y: f32 }
    ```

    `dt_poly`, `rec_poly`, `word_boxes`, `label` and `orientation_angle` do **not** cross the boundary — the first two are redundant with `bounding_box` for this use, `word_boxes` is unpopulated (`return_word_box = false`), `label` is unused, and `orientation_angle` is always `None` in our configuration. **No whole-text accessor is added to core** (AD-1: joining regions into one string for a Copy button is presentation); the view derives it. `src/tools/ocr/ocrOutcome.ts` is updated as the hand-synced TS mirror of the whole shape. The change is **mostly subtraction** — the adapter stops discarding what it already builds — plus one deliberate addition. (**Amended at code review, 2026-09-08.** As written this said "not new computation", and the character-measurement round made that false: `measure_char_polygons` crops each region, computes an Otsu threshold, builds a column-wise ink profile and derives the N−1 widest gaps as word separators. That is real new computation, it earns its place — it is what makes a find highlight land on the characters it matched rather than on the line containing them — and it added a fourth field to the struct above. The rationale survived in the Completion Notes; the AC was never updated, which is exactly 8.6's lesson 4 recurring. `polygon`, `text` and `confidence` remain pure subtraction).
...
**Reconciled at code review, 2026-09-08.** The set difference was computed in both directions against `git diff --name-status main...HEAD`, and the six rows marked *Added at code review* below were shipped but unlisted. Every one is substantively inside the OCR island or a direct consequence of a review patch, so nothing here is a scope breach — but this table's own rule is that an unlisted file "is an AC gap to be raised, not absorbed", and they were absorbed. Also checked and clean in the other direction: `src/shell/icons.spec.ts` is listed and untouched, correctly — it iterates `ALL_ICON_NAMES` derived from `icons.ts`'s own map, so it covers `ocr`/`pdf`/`image` without editing.
...
| `src/tools/ocr/overlayGeometry.ts` (+ spec) | **Added at code review, 2026-09-08** — AC34's placement maths, extracted so the geometry is unit-testable without mounting the view |
| `src/tools/ocr/findMatches.ts` (+ spec) | **Added at code review, 2026-09-08** — AC38's match scan |
| `src/tools/ocr/lowConfidence.ts` (+ spec) | **Added at code review, 2026-09-08** — AC36's confidence classification |
| `src/tools/ocr/realOutcome.fixture.json` + `realOutcome.spec.ts` | **Added at code review, 2026-09-08** — a real serialised outcome, so the TS mirror is checked against Rust's actual output rather than a hand-written fixture |
...
| `src/shell/toolError.spec.ts` | **Added at code review, 2026-09-08** — AC26's table test; the surface listed only `toolError.ts` | |
| `src/shell/invoke.ts` | **Added at code review, 2026-09-08** — `runLatestWins` passes an `isLatest()` predicate into the task, so a task that publishes mid-flight can decline a stale write | |
...
38. **Given** the first job named was finding an error code in a dense screenshot, **when** `⌘F` is pressed with the OCR view active, **then** a find field opens in the **same top-right cluster** as Copy, filtering the recognised text and marking matches **on the image**: a match highlights the **matched substring**, not the whole line — positioned from the per-character geometry core measures from the ink (`char_polygons`), the same source AC34's spans are fitted against, so it carries a selection highlight's accuracy rather than introducing a second, less honest mechanism. **When core declines to measure a region** — tilt beyond ~2°, fewer columns than characters, gaps it cannot resolve — the match bands the whole region and is marked `approximate` (dashed outline, no fill), the same honest-imprecise posture the measurement itself takes. (**Amended at code review, 2026-09-08:** this read "derived from AC34's width-fitted span", written before the character-measurement round existed; the shipped mechanism is the shared `char_polygons` source, and the whole-region fallback was never in the AC at all.) A match takes `--color-accent-signature-tint`, and the **current** match adds a solid 2px `--color-accent-signature` outline so "which one am I on" does not rest on tint alone. A match count is shown; `Enter` / `⇧Enter` step forward and back; `Esc` closes. **The handler is scoped to this view, not intercepted app-wide** — no other tool has a find, and a global interceptor would be shell surface this story does not need to claim. Match order follows the same reading-order sort as everything else (AC21).
...
40. **Given** an error must explain without removing the way back in, **when** an extraction fails, **then** the message renders in a `role="alert"` region **inside the resting drop target, which stays on screen** — the tool does not become a dead end. (**Amended at code review, 2026-09-08:** this read "below". `.pane` is `flex: 1`, so a message placed after the drop target landed against the bottom edge of the window, far from the thing it was about; it renders within the target instead, where the eye already is. The requirement it encodes — that the way back in is never removed — is unchanged.) The message is the project-authored sentence from AC24 or AC25, never a third-party error string.
...
Adversarial code review, 2026-09-08 (`bmad-code-review`, three parallel layers: Blind Hunter,
Edge Case Hunter, Acceptance Auditor). Target: `git diff -M main...HEAD`, code paths only.
40 raw findings across the three layers -> 30 after dedup and triage; 4 dismissed.
Severities below are the reviewer's own, assigned after reading each call site (the layers'
own ratings are deliberately discarded — they review under information asymmetry).

#### Decision needed — all five resolved by the developer, 2026-09-08

Each ruling below converts its finding into a patch. Recorded here rather than in the patch list
so the decision and its alternatives stay legible next to the finding that forced it.

- [x] [Review][Decision] **AC37's ⌘A keyboard path is unreachable as built** — `OcrView.vue:452` gates on `overlayEl.contains(document.activeElement)`, but the overlay is a `role="group"` with no `tabindex` and its children are plain `span`/`div`, so `document.activeElement` is `<body>` (or the find input) at all times and the branch never runs. Verified by grep: no `tabindex`/`contenteditable` anywhere in the file. Neither ⌘F nor ⌘A has any test coverage. Options: (a) make the overlay focusable (`tabindex="0"`), which changes tab order and screen-reader semantics and should be checked in AC30's owed VoiceOver pass; (b) drop the focus condition and let ⌘A select the overlay whenever a result is shown; (c) retire AC37's keyboard clause and rely on the Copy control. **RULED: make the overlay focusable.** Add `tabindex="0"` to the overlay so ⌘A behaves as AC37 describes, and add coverage for both ⌘F and ⌘A. The focus-order and screen-reader consequences are to be confirmed in AC30's owed VoiceOver pass, not assumed from the DOM.
- [x] [Review][Decision] **`OcrView.vue` is the first tool island to register a window-level listener, against AD-14** — `OcrView.vue:471` `window.addEventListener("keydown", onKeydown)`, removed on unmount at `:479`. Verified by grep across `src/tools/` and `src/components/`: every other `addEventListener` is shell-level (`AppPopover.vue`, `DropZone.vue`, `CommandPalette.vue`). The listener is mount-scoped so AC38's "not intercepted app-wide" holds in effect, but AD-14's rule is about *who registers*, and the spine amendment this story made covers only the asset protocol. Options: (a) record a deliberate AD-14 exception in `ARCHITECTURE-SPINE.md`; (b) move ⌘F/⌘A dispatch into the shell alongside ⌘K/⌘V; (c) scope the listener to the view's own root element. **RULED: record a spine exception.** Amend `ARCHITECTURE-SPINE.md`'s AD-14 to permit a mount-scoped `window` keydown listener for view-local shortcuts, naming `OcrView.vue` as the precedent and the mount-scoping as the condition. The listener stays where it is.
- [x] [Review][Decision] **AC9 is violated: 2 of the 8 "Image section" test blocks were rewritten during a move AC9 requires to be byte-identical** — `ImageView.spec.ts:118` and `:139`. Verified by diffing `git show main:src/tools/bucket/BucketView.spec.ts` against the new file: exactly two `it()` titles changed (`"…without disturbing OCR/PDF state"` -> `"…the Image view's own alert"`) and four assertion/setup lines were dropped from each. The other 6 Image blocks and **all 13 PDF blocks are byte-identical**. The reasoning is sound — there is no OCR section left to disturb — but AC9's own text says a changed assertion "is a failed AC9, **not a judgement call**". Options: (a) rule the rewrite correct and amend AC9 to carve out assertions whose subject no longer exists; (b) restore the two blocks verbatim and let them assert against the now-absent state; (c) move both to `OcrView.spec.ts` where the cross-tool isolation claim still has a subject. **RULED: the rewrite is correct; amend AC9.** Carve out assertions whose subject no longer exists after the split, so the move gate keeps its teeth for everything else. The two blocks stay as written.
- [x] [Review][Decision] **Four Task-2 decisions changed the code but never reached the amendment ledger** — the ledger at `8-7-reimagine-the-bucket-ocr.md:1087–1092` lists AC19, AC34, AC36, AC37/AC38 and AC42. Missing: **AC16** still reads "the change is **subtraction** … not new computation" while `ocr.rs:260` `measure_char_polygons` crops, Otsu-thresholds, builds a column ink profile and derives N−1 word gaps, and `OcrRegion` carries a fourth field (`char_polygons`) the stated struct does not contain. **AC38** still says a match highlight is "derived from AC34's width-fitted span", but matches are positioned from core's `char_polygons` (`ocrOutcome.ts:29–39`) — a second mechanism — and fall back to banding the whole region (`OcrView.vue:917`). **AC40** says the error renders "below the resting drop target"; it renders inside it (`OcrView.vue:663–672`). **AC8** says PDF/Images styles transplant "unchanged"; the `.pdf-section`/`.image-section` `border-top` rules were deliberately dropped and `<h2>` promoted to `<h1>` (`PdfView.vue:177,328–335`, `ImageView.vue:169,248–253`). All four changes are right and three are reasoned in-file or in the Change Log; none amended its AC. This is the "rationale died, decision survived" pattern the story itself cites as 8.6's lesson 4. Options: (a) amend all four AC texts and add them to the ledger; (b) add ledger rows only, leaving AC text as the historical record. **RULED: amend all five AC texts and add ledger rows** — AC16, AC38, AC40, AC8 here plus AC21 below — so the ACs describe what shipped and stay usable as a forward reference for 8.8/8.9.
- [x] [Review][Decision] **AC21 and AC23 cannot both be satisfied; the code chose AC23** — AC21 requires the ordering corpus test to assert "the **full ordered string**, not `contains`"; AC23 requires "per-line containment against an expected ordered line list rather than byte equality, since OCR output is not byte-exact". `ocr.rs:1240,1259,1292,1337` all use `assert_order(&outcome, &[…])` — relative order of a chosen subset. AC21's *intent* (ordering asserted distinctly from content) is fully met; only its literal phrasing is unsatisfiable. Options: (a) amend AC21's wording to "asserts relative order of a named subset"; (b) leave both and record the contradiction. **RULED: amend AC21's wording** to "asserts the relative order of a named subset", with a ledger row recording that AC23's tolerance model is the binding one.

#### Patch — all 23 applied 2026-09-08

- [x] [Review][Patch] **HIGH — the asset-scope grant stores a non-canonical path while the protocol checks a canonicalized one** [`src-tauri/src/commands/ocr.rs:159`] — verified against the vendored `tauri-2.11.5/src/scope/fs.rs`: `allow_file` (`:369`) stores the path **as given** via `push_pattern`, but `is_allowed` (`:420`) runs `try_resolve_symlink_and_canonicalize` **before** matching. Verified on this machine that `/var -> private/var` and `/tmp -> private/tmp`. So a file dragged out of Safari, Preview or Quick Look — which lands in `/var/folders/…/TemporaryItems/`, the single most common macOS drag source — is granted as `/var/folders/…` and checked as `/private/var/folders/…`, matching nothing: the asset request 403s. The OCR text still arrives and the overlay spans still render, over an empty rectangle, with no error. Fix: grant `std::fs::canonicalize(&path)` (or both forms).
- [x] [Review][Patch] **HIGH — `pasteSourceImage` is published from inside the runner task, so pixels and result are ordered by different clocks** [`src/shell/DropZone.vue:47`] — the drop path publishes `dropSourcePath` **synchronously before** `runLatestWins` (`:125`), but the paste path publishes `pasteSourceImage` **inside** the task, after `await readClipboardImage()`, unguarded by `superseded`. Two reachable outcomes. (1) Paste A, then drop B while A's clipboard read is pending: A's image publishes late, the view's watcher calls `resetForNewSource()` and paints B's regions over A's pixels — geometry from one image on another, silently, no error. (2) If A publishes after B's outcome, `resetForNewSource()` nulls `outcome`, A's invoke is superseded so `pasteResult` is never written, and `extracting` (`OcrView.vue:69`, `hasImage && outcome === null && error === null`) is **true forever** — a permanent spinner with no exit. Same failure on two rapid ⌘V presses whose clipboard reads resolve out of order. `dropZone.spec.ts`'s latest-wins tests interleave the *invoke* promises but never the clipboard read, so the seam is untested by construction. Fix: re-publish `pasteSourceImage` from `result.value.image` inside the existing `if (!result.superseded && isStillActive())` block, so the final state always matches the final result.
- [x] [Review][Patch] **The grant races the `<img>` request it exists to authorize, and a lost race is permanent** [`src-tauri/src/commands/ocr.rs:110–118`, `src/tools/ocr/OcrView.vue:358`] — `grant_asset_access` runs **inside** `spawn_blocking`, after `check_file_size`, while `imageSrc = convertFileSrc(path)` is set before the `invoke` on the picker path and in the same tick on the drop path. Nothing orders a custom-scheme handler call against a blocking-pool dispatch plus a `metadata()` stat. If the image request wins, the protocol 403s and the `<img>` `src` never changes, so the browser never retries. Fix: grant synchronously in the command body before `spawn_blocking` (same change site as the canonicalization fix).
- [x] [Review][Patch] **The overlay measures text in one font and renders it in another** [`src/tools/ocr/overlayGeometry.ts:24`, `src/tools/ocr/OcrView.vue:874`] — `OVERLAY_FONT_STACK` is the system stack (`-apple-system, BlinkMacSystemFont, …`) and its own doc says it is "exported so the two cannot drift"; `.region` renders `font-family: var(--font-sans)`, which `tokens.css:5–16` resolves to the bundled `"Geist Sans"` webfont. `fitLetterSpacing` solves `(target − naturalSF)/len` but the span renders at `naturalGeist + spacing·len`, leaving an unabsorbed residual that accumulates monotonically left-to-right — so a native drag-selection highlight, the entire premise of Live Text, drifts off the words toward the end of every line. Invisible to the suite: jsdom has no canvas 2d context, so `measure` returns `NaN` and `letterSpacingPx` is 0 in all 963 tests. Fix: render `.region` with `OVERLAY_FONT_STACK`, or point the constant at `--font-sans`.
- [x] [Review][Patch] **No `@error` handler on the source `<img>`** [`src/tools/ocr/OcrView.vue:683–688`] — only `@load` is bound. This is the amplifier that makes the two findings above silent: a failed grant (which `ocr.rs:159` deliberately swallows to `eprintln!`), a 403, or a file moved between drop and render all produce a blank rectangle with transparent text and dashed unreadable-boxes floating on it, and no way for the user to tell whether OCR failed or the render did. `onImageLoad` never fires, so `loadedSize` stays null and the surface is sized from `outcome.image_width/height` regardless. No test mounts an image that fails to load.
- [x] [Review][Patch] **The error branch never updates the live region** [`src/tools/ocr/OcrView.vue:255–262`] — `applyOcrResult`'s `if ("error" in result)` branch sets `outcome`/`error`/`imageSrc`/`pastedImage` and returns **without touching `announcement`**, and because it clears the image, `extracting` goes false without the watcher firing. So after any failure the always-mounted `.sr-only` announcer still reads "Extracting text…" or, worse, the previous run's "Text extracted." The visible error is a `v-if`-inserted `role="alert"` (`:669`) — exactly the pattern the code documents at `:266–273` as producing no announcement, which is why the announcer exists. This is the same bug class AC30's manual pass caught for the no-text case, left unfixed for the error case.
- [x] [Review][Patch] **An outcome applied to a view with no image renders nothing and announces success** [`src/tools/ocr/OcrView.vue:255–283`, `:499`, `:642`] — verified that every result surface (error, no-text, findbar, copy, the overlay) sits inside `v-if="hasImage"` or the `v-else` `.surface`. Drop an image, navigate away, navigate back before the ~3 s inference resolves: the old instance consumed and nulled `dropSourcePath`, the new instance's watcher is not `immediate` so it never sees it, but `DropZone`'s `isStillActive()` checks the **route**, not the instance, so `dropResult` is written anyway. `applyOcrResult` sets a real outcome, renders nothing at all, and sets `announcement` to "Text extracted." — the tool bluffing about its own result in the one place the story went furthest to prevent it.
- [x] [Review][Patch] **A ⌘V with a non-image clipboard destroys a good result and shows untranslated third-party prose** [`src/shell/DropZone.vue:55–59`, `src/shell/toolError.ts:139`] — `readClipboardImage()` rejects, the resulting `ToolError` carries no `ocr-*` code, so `toolErrorMessage` falls through to the raw plugin string, and the error branch wipes the existing outcome and image back to the resting drop target. The story translated `ocr-unsupported-format` because it is "the single most-hit error in this tool"; copy-a-URL-then-⌘V is at least as likely, and is neither translated nor non-destructive. Fix: give the clipboard-read failure its own `ocr-*` code with `en`/`fr` entries, and leave the existing result standing.
- [x] [Review][Patch] **Character polygons are indexed by Rust `char`, consumed at JS UTF-16 offsets** [`crates/umbra-core/src/ocr.rs:283`, `src/tools/ocr/findMatches.ts:38–40`, `src/tools/ocr/overlayGeometry.ts:97–107`] — Rust emits one polygon per Unicode scalar (`text.chars()`); `findMatches` derives `start`/`end` from `String.indexOf` on a `toLowerCase()`ed copy, i.e. UTF-16 code units on a string that may not even share the original's length. Any astral-plane character before a match shifts every later index by +1 per surrogate pair, and length-changing case folds (`İ` -> `i̇`) desynchronise the haystack from `text` outright. `charRunPlacement` then returns a plausible, wrong box rather than `null`, so it is not even flagged `approximate`. `realOutcome.spec.ts:21` compares against `[...text].length` (code points), so it cannot detect the mismatch, and every `findMatches.spec.ts` fixture is ASCII in a bilingual EN/FR app.
- [x] [Review][Patch] **Leading and trailing whitespace in a region gets degenerate `(0,0)` spans pinned at the region's left edge** [`crates/umbra-core/src/ocr.rs:397–420`] — `spans` is initialised to `(0.0, 0.0)` for every character; the word loop fills `first..last` and the separator block fills the gaps *between* words, but `separators` has exactly `words.len() − 1` entries, so indices before `words[0].0` and after `words.last().1` are never assigned. Those characters emit as zero-width quads at `x0`. A find run touching one makes `charRunPlacement` build a quad whose `last` box sits at the left edge, giving `atan2(0, x0 − first.x) = π` — a match band drawn rotated 180° and extending backwards across the line. The doc comment promises index-alignment; it is length-aligned but positionally false at both edges. Reachability depends on whether the CTC decoder emits padded text — the mishandling path is verified, the trigger is not.
- [x] [Review][Patch] **An empty or sub-4-point polygon collapses the whole page into one reading-order row** [`crates/umbra-core/src/ocr.rs:173–188`] — `vertical_bounds` folds from `(f32::MAX, f32::MIN)`, so an empty polygon returns `(MAX, MIN)`; `polygon_height` then computes `(MIN − MAX).abs()`, which overflows f32 to `+inf`, making `tolerance = 0.5 · inf = inf` in `sort_into_reading_order` — every region bands into one row and the page re-sorts purely left-to-right, breaking Copy fidelity, NFR5 screen-reader order and find ordering at once. `measure_char_polygons` guards `polygon.len() < 4`; the sort does not, and the TS side's `quadPlacement` does. Polygons come straight from `region.bounding_box.points` with no filter (`ocr.rs:595–605`), so reachability is near-nil but the guard is two lines and the failure is total.
- [x] [Review][Patch] **`realOutcome.spec.ts` asserts an invariant the Rust side explicitly does not guarantee** [`src/tools/ocr/realOutcome.spec.ts:21`] — it requires `char_polygons.length === [...text].length` for **every** region, but `measure_char_polygons` returns an empty vector by design whenever measurement is not confident (tilt, too few columns, unresolved gaps — `ocr.rs:277–322,380,400`), which is the documented "honest imprecise" fallback that `declines_to_measure_characters_on_a_photographed_page` asserts. The spec passes only because the checked-in fixture is a clean screenshot; regenerate it from a photo and the test fails for correct behaviour. Fix: `length === 0 || length === charCount`.
- [x] [Review][Patch] **Any dropped file is granted webview read access before it is validated as an image** [`src-tauri/src/commands/ocr.rs:112–118`] — the order is `check_file_size` -> `grant_asset_access` -> `read_file_bytes` -> decode/format validation. Drop `id_rsa`, a `.docx` or a PDF and it is permanently on the process's asset allow-list even though extraction fails immediately and the view never renders it. The function's own doc accounts for session-scoped accumulation of *images the user opened*, not of files the tool rejected, so the standing grant is wider than the comment claims. Only reachable with webview script injection, but the ordering is free to fix.
- [x] [Review][Patch] **A failed grant is reported to stderr only, on a path the user can observe failing** [`src-tauri/src/commands/ocr.rs:159–161`] — `eprintln!` into a stream a packaged `.app` discards. Not failing the extraction is the right call, but the user-visible consequence (a permanently blank pane with text floating over it) reaches them with no explanation at all; the codebase otherwise routes shell-side diagnostics through `console.error`.
- [x] [Review][Patch] **`drawPastedImage` throws unguarded on an RGBA length mismatch** [`src/tools/ocr/OcrView.vue:327,335–345`] — `new ImageData(new Uint8ClampedArray(image.rgba), width, height)` throws `DataError` when `rgba.length !== width * height * 4`, and the call site is a bare `void drawPastedImage()` with no `.catch`. The failure surfaces only as an unhandled rejection; the canvas stays blank and the view depends entirely on Rust independently rejecting with `ocr-malformed-image-buffer`. Reachable only if the clipboard plugin returns inconsistent `rgba()`/`size()` values, but the guard is one line.
- [x] [Review][Patch] **Three no-text-found sub-cases lost their coverage, beyond AC29's "only these two" retirements** [`src/tools/ocr/OcrView.spec.ts:347`] — the baseline had three distinct no-text tests: empty drop (`BucketView.spec.ts:134`), empty **paste** (`:151`) and **whitespace-only** outcome (`:168`). The new suite has one generic empty case plus the AC39 diagnosis. Verified by grep that no whitespace-only test exists on either side (`has_readable_text` has five assertion sites in `ocr.rs`, none of them a whitespace case). The trimming behaviour is implemented (`ocr.rs:100`, `OcrView.vue:75`), so this is lost coverage rather than lost behaviour — but the retirement is silent, which is precisely what AC29's clause exists to prevent.
- [x] [Review][Patch] **Seven files breach the normative *Authorised file surface* table without being flagged** [`8-7-reimagine-the-bucket-ocr.md:128`] — the table's **New** rows list only `OcrView.vue`, `OcrView.spec.ts` and `ocrOutcome.ts` under `src/tools/ocr/`, but six more shipped: `overlayGeometry.ts`(+spec), `findMatches.ts`(+spec), `lowConfidence.ts`(+spec), plus `realOutcome.fixture.json` and `realOutcome.spec.ts`. `src/shell/toolError.spec.ts` (+31 lines, the AC26 table test) is modified while the table lists only `toolError.ts`. The File List enumerates them but marks only `epics.md` and `Cargo.lock` with ⚠. Every one is substantively inside the OCR island and justified; the gap is procedural, and the table's own rule says an unlisted file "is an AC gap to be raised, not absorbed".
- [x] [Review][Patch] **AC10's "nine remaining `bucket_*` commands" is arithmetically wrong** [`8-7-reimagine-the-bucket-ocr.md`, AC10] — `src-tauri/src/lib.rs:69–73` registers exactly **five**: `bucket_merge_pdfs`, `bucket_extract_pdf_pages`, `bucket_extract_pdf_text`, `bucket_convert_image`, `bucket_estimate_image_size`. All five are untouched, which is what AC10 actually requires; only the count is wrong.

#### Deferred

- [x] [Review][Defer] **Two competing polite live regions once there is text to find** [`src/tools/ocr/OcrView.vue:567–570`] — deferred to AC30's owed manual VoiceOver pass. The `.match-count` span carries `role="status"` alongside the `.sr-only` announcer, so typing in the find field updates one polite region on every keystroke while the other holds the extraction result. The spec's "exactly one polite live region is speaking the result" is asserted only against `EMPTY_OUTCOME` — the one configuration where the find bar is not rendered — so the assertion cannot fail where the second region actually exists. Whether this is harmful or desirable is a screen-reader question, not a code-reading one.
- [x] [Review][Defer] **Latest-wins bounds delivery, not work: rapid inputs start unbounded concurrent inferences** [`src-tauri/src/commands/ocr.rs:69–80`] — deferred, pre-existing app-wide AD-16 shape, not introduced by this story. N rapid drops/pastes spawn N concurrent `spawn_blocking` ONNX inferences over the one shared `OAROCR`, with no semaphore, queue cap or cancellation; the module comment scopes the concern to "two inference jobs genuinely overlap" but nothing bounds it at two. On a full-screen Retina capture (13 s by the story's own measurement) a handful of impatient re-drops saturates the CPU with work whose results are all discarded.
- [x] [Review][Defer] **`run_ocr` clones the full decoded image, doubling peak memory on the largest permitted inputs** [`crates/umbra-core/src/ocr.rs:570`] — deferred, needs restructuring rather than a patch. `MAX_INPUT_BYTES` is 100 MB of *compressed* bytes, which can decode to several hundred MB of `RgbImage`; the clone exists only because `predict(vec![image])` takes ownership and `measure_char_polygons` needs the pixels afterwards. Measuring against a borrowed buffer, or before handing the image over, would remove it.
...
| Date | Change |
| --- | --- |
| 2026-09-08 | **Code review (`bmad-code-review`, three parallel layers) — 23 patches applied, 5 decisions ruled, 3 items deferred.** Two findings rated high, both of the same shape: a correctness property depending on two components agreeing with nothing enforcing the agreement. (1) `allow_file` stores a path literally while `is_allowed` canonicalizes before matching, so on macOS every file dragged out of Safari, Preview or Quick Look — `/var/folders/…`, checked as `/private/var/folders/…` — was granted under one name and refused under another; the grant now covers both forms, is awaited by the view through a new `ocr_grant_asset` command so it cannot lose the race to the `<img>` request, and is made only for a file that passes the size guard and opens with a decodable signature. (2) `pasteSourceImage` was published from inside the runner task, ordered by clipboard-read completion while results were ordered by dispatch — two channels, two clocks, reachable both ways: paint one image's regions over another's pixels, or strand the view in a permanent in-flight state. `runLatestWins` now passes an `isLatest()` predicate into the task. Also: the overlay measured in the system font and rendered in Geist Sans, so selection drifted off the words toward the end of every line (invisible to the suite — jsdom has no canvas context, so letter-spacing was 0 in all 963 tests); the error branch never touched the announcer, leaving a screen reader on "Extracting text…" after a failure — the same hole AC30's pass closed for the no-text case; the `<img>` had no `@error`, which is what made all of the above silent; a result delivered to a view that never saw its source rendered nothing while announcing success; a ⌘V with a non-image clipboard destroyed a good result and showed untranslated plugin prose; find offsets were UTF-16 code units indexing a scalar-aligned array; leading and trailing whitespace got zero-width spans pinned at the region's left edge; an empty polygon made the row-banding tolerance infinite. Gate: **974 Vitest** (from 963) / **373 cargo** (from 371), plus `pnpm lint`, `vue-tsc --noEmit`, `pnpm build`, `cargo fmt --check` and `cargo clippy --workspace --all-targets -- -D warnings`, all clean. **Owed before merge:** a `pnpm tauri dev` render-review in light and dark, and a VoiceOver re-check — the patches changed real UI surface that no manual pass has seen: the overlay is now focusable (`tabindex="0"`, which puts it in the tab order), every region span renders in a different font than before (the measurement font, which is the point), and two error states that previously rendered nothing now render and announce. AC30's pass predates all three. |
| 2026-09-08 | **Amendment ledger — six ACs corrected at the code review** (developer's ruling: amend the text, not just record the divergence, so the ACs stay usable as a forward reference for 8.8/8.9). **AC8** — two deviations from "transplanted unchanged" authorised, both forced by the split: the `.pdf-section`/`.image-section` `border-top` rules dropped, `<h2>` promoted to `<h1>`. **AC9** — one narrow carve-out from the byte-identical move gate: an assertion whose *subject no longer exists after the split* may be dropped, which covers exactly the two Image blocks that asserted an Images error did not disturb OCR/PDF state inside the shared view. All 13 PDF blocks and the other 6 Image blocks verified byte-identical. **AC10** — "nine remaining `bucket_*` commands" corrected to five. **AC16** — "the change is subtraction, not new computation" was made false by the character-measurement round (Otsu thresholding, ink profiling, word-gap derivation) and by the fourth struct field `char_polygons`, neither of which the AC named. **AC21** — "asserts the full ordered string" was unsatisfiable alongside AC23's per-line containment; reworded to the relative order of a named subset, which is what `assert_order` does and what the AC actually intended. **AC38** — match placement derives from `char_polygons`, not "AC34's width-fitted span", and bands the whole region marked `approximate` when core declines to measure. **AC40** — the error renders *inside* the resting drop target, not below it (`.pane` is `flex: 1`, so "below" put the message at the window's bottom edge). |
| 2026-09-08 | **AD-14 exception recorded in `ARCHITECTURE-SPINE.md`** (developer's ruling at code review). `OcrView.vue` is the first tool island to register a `window`-level `keydown` listener. The rule now permits one, mount-scoped and claiming only keys no other tool uses — its purpose is to stop shortcuts colliding *across tools*, and a listener that exists only while its own view is mounted cannot. The same review found AC37's ⌘A branch **unreachable as shipped**: it gated on the overlay containing `document.activeElement`, but the overlay had no `tabindex` and only non-focusable children, so the condition was never true and ⌘A fell through to the browser default. The overlay now carries `tabindex="0"`; the focus-order and screen-reader consequences are owed to AC30's next pass. Neither ⌘F nor ⌘A had any test coverage; both have it now. |
