...
- [x] [Review][Patch] `MUST_CONVERT` never exercises the minute-step `step == 1` special-case wording (`"every 1 minute"` → `"every minute"`) via the NL path — Task 2 explicitly requires "both minute-step and hour-step forms, including the `step == 1` special-case wording," but only the hour form (`"every 1 hour"`) is tested. The minute analog is a distinct, independently reachable branch in `time_clause` (`(FieldSpec::Step{base:Wildcard,step:1}, FieldSpec::Wildcard)`), verified reachable via `parse_time_step`. [crates/umbra-core/src/cron.rs:1296]
- [x] [Review][Patch] The documented comma-after-single-weekday grammar gotcha (e.g. `"every Monday, every 2 hours"` fails to parse while the comma-less form succeeds — flagged in this story's own Dev Notes as a real trap for future contributors) has no `MUST_HONESTLY_FAIL` row pinning it as a permanent regression guard, despite the corpus's stated purpose being exactly this. [crates/umbra-core/src/cron.rs:1339]
- [x] [Review][Patch] Neither corpus test function asserts `MUST_CONVERT.len() >= 30` or `MUST_HONESTLY_FAIL.len() >= 10` as a structural invariant — AC1/AC2's stated minimums live only in this story's prose, not in the code that's supposed to be the "executable definition of correct." A future accidental trim of either table would pass CI silently. [crates/umbra-core/src/cron.rs:1201]
- [x] [Review][Patch] Completion Notes and Testing Requirements label the `umbra_core` crate's own test count (109→112) as the `cargo test --workspace` total; the true workspace total is 152→155 (adds `umbra_lib`'s 43 tests, confirmed by independently running `cargo test --workspace`). Functionally harmless — the underlying +3 delta claim is correct — but a factual inaccuracy in evidence this story asks a reader to trust, and it repeats imprecise phrasing inherited from Story 3.2. [story file: Testing Requirements / Change Log]
- [x] [Review][Patch] `MUST_HONESTLY_FAIL` tests only interior over-range step values (`"every 90 minutes"`, `"every 30 hours"`); the exact field-range boundary (60 minutes, 24 hours) is untested, so an off-by-one regression at the boundary itself would not be caught. [crates/umbra-core/src/cron.rs:1349-1350]
- [x] [Review][Defer] The must-honestly-fail leak-check only matches the literal substring `"* *"`, so a leaked partial cron fragment without two adjacent asterisks (e.g. `"*/90"`, `"9-17"`) would pass undetected — deferred, pre-existing convention explicitly reused from Story 3.2 per this story's own Task 3 instructions, not introduced by this diff. [crates/umbra-core/src/cron.rs:1400-1404]

`decision-needed`: 0 · `patch`: 5 · `defer`: 1 · dismissed as noise: 4 (duplicate must-convert/must-honestly-fail phrases vs. Story 3.2 — explicitly justified as intentional in Dev Notes; weaker per-row assertions on those duplicates — no coverage regression since Story 3.2's stricter tests remain; Task 4's round-trip test framed as catching more than `build_result`'s existing runtime self-check already guarantees — architectural observation, not a defect; and one verified false positive — an "untested 4th am/pm marker variant" claim that doesn't hold once `parse_ampm`'s `trim_start()` behavior is read, since leading-space handling isn't a separate code branch from the dotted/undotted marker check).
...
- `cargo test --workspace` must show the new corpus tests passing, with the passed-test count visibly higher than the pre-story baseline (Story 3.2 landed at 109 passed for the `umbra_core` crate's test binary, one of several binaries `cargo test --workspace` runs — per that story's Change Log — this story should add roughly 2-4 new `#[test]` functions, whose bodies each loop over ~13-31 corpus rows via assertions, not ~44 individually-named test functions).
...
- **Branch already created:** Stories 3.1 (#32) and 3.2 (#33) merged to `main` after this story's initial draft but before Task 8 executed — `main`'s tip is this story's `baseline_commit` (`f8c2577`), which already includes both stories' full implementation and code-review patches. `feat/story-3-3-the-phrase-corpus-as-an-automated-acceptance-gate` branches directly from `main` (no stacking needed, unlike Story 3.2 off 3.1), and already carries this story's own first commit.
...
- `umbra_core` crate tests (one binary within `cargo test --workspace`): 109 → 112 passed (+3, matching the 3 new test functions — sanity-checks that the loop bodies actually execute over the corpora rather than iterating an empty array). Full `cargo test --workspace` total across all binaries: 152 → 155.
