...
1. **Given** a supported English phrase (e.g. "every weekday at 8:30am"), **when** I submit it, **then** the correct 5-field cron expression is produced by a deterministic grammar parser in `umbra-core::cron`, fully offline (FR19, INV-1), **and** before display, the result is round-tripped through Story 3.1's cron→English direction (`describe()`); only a consistent round-trip is shown (AD-9).
...
- [x] [Review][Decision] AC1's own example phrase ("every weekday at 8:30") was not actually convertible under the implemented, Task-3-compliant grammar (AM/PM mandatory, never inferred). Resolved: corrected AC1's wording to "every weekday at 8:30am", matching Task 3's grammar and Task 7's actual tests. [crates/umbra-core/src/cron.rs Task 3 grammar; story AC1 text]

- [x] [Review][Patch] `detect_ambiguous_time`'s unanchored substring search (`lower.find("at ")`) fabricates a false "understood a time" diagnosis for ordinary words ending in "at" — empirically confirmed: `parse_schedule("chat 9")` and `parse_schedule("format 9")` both wrongly return `cron-nl-ambiguous-time` with a fabricated "Understood a time of 9:00..." message, even though no time expression was present. Also only checks the first "at " occurrence, so a genuine ambiguity later in a longer phrase can be missed if an earlier spurious match short-circuits the scan. Directly undercuts AD-9/AC2's honesty guarantee. Fixed: word-boundary-anchored scan over all "at " occurrences, with regression tests. [crates/umbra-core/src/cron.rs:770-780]

- [x] [Review][Patch] `parse_time_step` never bounds-checks the parsed step against the field's valid range (minutes 0-59, hours 0-23) — empirically confirmed: "every 90 minutes" generates `*/90 * * * *`, described as "every 90 minutes," but its real `next_runs` fire exactly 60 minutes apart (hourly); "every 30 hours" generates `0 */30 * * *`, described as "every 30 hours," but fires 24 hours apart (daily). This is exactly the "wrong-but-confident" bug class AD-9 exists to prevent, and the round-trip check can't catch it because it only compares two descriptions built from the same struct, never the struct against real cron semantics. Fixed: step values outside the field's range are rejected as honest failures, with regression tests. [crates/umbra-core/src/cron.rs:723-765]

- [x] [Review][Patch] Several failure paths downstream of a recognized day clause inconsistently drop or misstate the "what was understood" context FR21 requires: (a) day clause + malformed trailing time (e.g. "every weekday at 25:00") falls to the fully generic message with `context: None`, unlike the sibling "day clause, no time at all" path; (b) day clause + ambiguous time (e.g. "every weekday at 8:30") produces an error mentioning only the time, never that the day clause was understood; (c) `parse_day_clause`'s trailing remainder is discarded (bound to `_rest`, never inspected), so a phrase like "every weekday sometime at 9am" is told "no time of day was given" even though a time was present, just unsupported. Fixed: `try_parse_full` now always returns a context-carrying result once a day clause is recognized (never falls through), with regression tests for all three sub-cases. [crates/umbra-core/src/cron.rs:401-467]

- [x] [Review][Patch] Duplicate weekday names in a list are accepted without deduping — e.g. "every Monday and Monday at 9am" produces cron field "1,1" and description "Every Monday and Monday, at 9:00 AM." Harmless (schedule still fires correctly) but easy to fix. Fixed: `parse_weekday_list` dedupes on push, with a regression test. [crates/umbra-core/src/cron.rs:624-652]

- [x] [Review][Patch] Two of Task 7's five must-honestly-fail tests (`..._out_of_scope_vocabulary_...`, `..._garbage_input_...`) only assert `err.code`, omitting the message/context assertions Task 7's own text specifies for every must-fail case. Fixed: added the missing message assertions. [crates/umbra-core/src/cron.rs:1036-1046]

- [x] [Review][Defer] `onPasteSchedule`'s catch branch never resets `parseResult`, leaving a stale prior successful conversion displayed alongside a new paste-failure alert — deferred, pre-existing (faithful mirror of Story 3.1's `onPaste`, which has the identical gap on `explanation`; fixing only the new section would create an inconsistency between the two now-mirrored sections). [src/tools/cron/CronView.vue:91-101]

- [x] [Review][Defer] `runParse`/`runPasteSchedule` are independent latest-wins runners with no cross-invalidation, so a slow Convert call can repopulate `parseResult` with a stale result after a subsequent Paste has already reset state — deferred, pre-existing (identical unaddressed race already present between Story 3.1's `onExplain`/`onPaste` on `explanation`; Story 3.2 faithfully mirrors the same pattern per Task 6's explicit instruction). [src/tools/cron/CronView.vue:22-30, 76-101]
...
- 2026-08-04: Code review (adversarial + edge-case + acceptance-audit layers) resolved 1 decision-needed and 5 patch findings, deferred 2, dismissed 10. Decision: corrected AC1's example phrase to "every weekday at 8:30am" (Task 3 correctly mandates AM/PM is never inferred; the original example was stale). Patches, each empirically verified before and after fix: (1) `detect_ambiguous_time` no longer fabricates an ambiguous-time diagnosis for ordinary words ending in "at" (e.g. "chat 9") — now word-boundary anchored and scans past false matches; (2) `parse_time_step` now rejects step values outside the field's valid range (0-59 minutes, 0-23 hours) — confirmed "every 90 minutes"/"every 30 hours" previously generated cron expressions whose real recurrence (hourly/daily) silently contradicted their own description, undetected by the round-trip check since it only compares two descriptions built from the same struct; (3) every failure path downstream of a recognized day clause now consistently names what was understood, fixing three sub-cases where that context was dropped or (in one case) actively misleading; (4) duplicate weekday names in a list are deduplicated; (5) two Task 7 tests gained the message assertions the task text specified. 9 regression tests added. Deferred (both pre-existing patterns faithfully mirrored from Story 3.1, not introduced by this story): `onPasteSchedule`'s catch branch not resetting `parseResult`, and the cross-runner Convert/Paste race on shared `parseResult` state — see `deferred-work.md`. Full verification pass re-run clean after patches: `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace` (109 passed). No frontend files touched by this patch round.
