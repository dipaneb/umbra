...
- [x] **Task 5: Deploy to Vercel**
  - [x] Developer connected `dipaneb/umbra-web` to Vercel (2026-08-09/10). Actual assigned URL: `https://umbra-web-beta.vercel.app` (`umbra-web` itself was already taken by another Vercel account — the placeholder guess from earlier in this session was wrong, as flagged). `astro.config.mjs`'s `site` corrected to match, rebuilt, verified (canonical links + sitemap + robots.txt all confirmed pointing at the real domain), committed, and pushed (`8208481`) — Vercel auto-redeployed from the push, confirmed live via `curl`.
