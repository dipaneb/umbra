...
- [ ] **Task 1: `umbra-core::uuid` — the generation function (AC: 1, 2, 3)**
  ...
  - [ ] Define the version selector as a typed enum, not a string, mirroring `JsonIndent` in `crates/umbra-core/src/json.rs` exactly (that's the established pattern for an enum-shaped parameter crossing the Tauri IPC boundary — Tauri deserializes it directly from the JS-side string, no manual parsing needed):
    ```rust
  ...
  - [ ] `crates/umbra-core/src/lib.rs`: add `pub mod uuid;` (insert alphabetically: `base64`, `error`, `json`, `uuid`).

- [ ] **Task 2: Tauri command `uuid_generate` (AC: 1, 2, 3)**
  - [ ] **New file `src-tauri/src/commands/uuid.rs`.** This tool has no file I/O, so `json.rs` (not `base64.rs`) is the closer template — copy its shape directly:
    ```rust
  ...
  - [ ] `src-tauri/src/commands/mod.rs`: add `pub mod uuid;` (insert alphabetically: `base64`, `json`, `uuid`).
  - [ ] `src-tauri/src/lib.rs`: add `use commands::uuid::uuid_generate;` and register `uuid_generate` in the `generate_handler![...]` list.
  - [ ] **No `src-tauri/Cargo.toml` change needed.** The `uuid` crate dependency lives in `umbra-core` only (AD-1: the transformation is core's job); `src-tauri`'s command file only imports the already-typed `UuidVersion` re-exported through `umbra_core::uuid`, it never constructs `uuid`-crate types directly — same reason `src-tauri` needs no direct dependency for `JsonIndent`.
  - [ ] **No `src-tauri/capabilities/default.json` change needed.** Like every existing custom `#[tauri::command]` in this project, `uuid_generate` is not a plugin-provided command, so it isn't subject to the capability ACL (confirmed precedent: `json_format`/`base64_encode` need no capability entries either).

- [ ] **Task 3: Tool Registry entry (AC: 1, 2, 3 — AD-5)**
  - [ ] `src/stores/registry.ts`: add a new entry to `TOOLS` (insert after the existing `base64` entry):
    ```ts
    {
      id: "uuid",
      name: "UUID",
  ...
  - [ ] This is the **only** registry change needed — per the store's own header comment, the sidebar, palette index (Story 1.6), and route table (`routes` computed) all generate from this one entry. Do **not** hand-edit `src/router/index.ts`, `src/shell/AppSidebar.vue`, or `src/App.vue` — none of them need to change for a new non-drop tool.

- [ ] **Task 4: `UuidView.vue` — the UI (AC: 1, 2, 3)**
  - [ ] **New file `src/tools/uuid/UuidView.vue` and `src/tools/uuid/uuidVersion.ts`** (the TS mirror of the Rust enum, same convention as `src/tools/json/jsonIndent.ts`):
    ```ts
