...
- FR28: Images: drag-and-drop or pick one or more images (source formats: PNG/JPEG/WebP), convert to PNG/JPEG/WebP/AVIF, resize, and compress with a quality slider showing a visual before/after — all locally, batch-capable. AVIF is an output format only in v1 — encoding is free via `image` 0.25's default `avif`/`ravif` feature (a pure-Rust AV1 encoder, already compiled in), but decoding an AVIF *source* needs the separate, non-default `avif-native`/`dav1d` feature (a C library) — checked against the crate's own feature graph, not assumed, and left out of this story's scope. A user-chosen background color replaces JPEG's hardcoded white alpha-flatten. EXIF/GPS metadata does not survive conversion — verified against the vendored `image` crate source and disclosed as a privacy property of the tool, not merely an accident of implementation. **Revised 2026-09-14 (Story 8.9):** open scope discovery (real persona/market research, not the developer's own usage) replaced the original single-file, no-resize scope below. HEIC re-examined per the standing dependency-drift discipline (Story 8.8's `lopdf` precedent): a new pure-Rust `heic-rs` crate (MIT/Apache-2.0, no C toolchain) clears the licensing wall that blocked every prior candidate, but is two days old as of this check (v0.1.1, 50 downloads) — not adopted this story; recorded so a future re-check starts from "clears licensing, check maturity" rather than a fresh crate search. See `8-9-images-decision-record.md`. Original scope (Story 6.2, 2026-08-10): HEIC descoped from v1 — every real Rust HEIC crate candidate investigated carried a concrete, unresolved blocker (AGPL/commercial dual license, unpublished/unconfirmed license, or a GPL/LGPL codec-dependency risk to the AD-11 CI compile gate). v1 shipped PNG/JPEG/WebP only; see Story 6.2's Task 1 for the full verification trail.
...
- FR28: Epic 6 — Image convert + compress with quality slider — **revised 2026-09-14 (Story 8.9)**: batch-capable, drag-and-drop, resize (real gap vs. every named market comparator), AVIF, configurable JPEG background, EXIF/GPS stripping disclosed as a verified privacy property. See `prd.md` FR28.
...
**Scope decided 2026-09-14 by this story's own Task 1** (`8-9-images-decision-record.md`). The developer rejected scoping the tool from their own usage and directed real persona/market research instead (freelance/backend/frontend/corporate/WordPress/student dev, plus desktop/mobile/game dev — a gap in the first research pass, corrected mid-session). Findings: **resize is a real, total gap** against every named market comparator (Squoosh, ImageOptim, TinyPNG, the WordPress optimizer plugins) — FR28 never mentioned it and the tool has none. Redesign adds **resize, AVIF as an output format (encode-only — reachable via the existing `image` crate's default `avif`/`ravif` feature; decoding an AVIF *source* would need the separate, non-default `avif-native`/`dav1d` C dependency, not part of this story), multi-file drag-and-drop, a visual before/after (replacing the bare size estimate), and a user-configurable JPEG alpha-flatten background** (hardcoded white since Story 6.2, deferred twice, developer's own reason: *"people often prefer black to white"*). **A genuine free find, verified against the vendored `image` crate source rather than assumed:** today's conversion already strips all EXIF/GPS metadata as a side effect of the decode/encode round-trip — this story turns that into a tested, disclosed privacy claim rather than new logic. **HEIC re-examined** per the standing dependency-drift discipline (Story 8.8's `lopdf` precedent): a new pure-Rust `heic-rs` crate clears the licensing wall every prior candidate hit, but at two days old (v0.1.1, 50 downloads) is **not adopted this story** — the developer's own call, recorded so a future re-check starts from "cleared licensing, check maturity." Registry `name` becomes **`"Images"`** (was provisional `"Image"`). **Four ideas cut to GitHub `backlog-candidate` issues** (developer's explicit route, all four): a dedicated vector/SVG optimization tool (a different asset class, likely its own future tool per the same reasoning that split "Bucket" into three), batch watermarking, a favicon/app-icon kit generator, and Story 8.7's own previously-unpicked "whole-image deskew" cut idea. **Video and game-dev texture formats (DDS/KTX2) are dropped outright, not backloged** — the developer's explicit call, video's cost being a categorically heavier native-dependency/licensing argument than anything this app has taken on (this project's own PDF-rasterizer story is the worked example of that cost class). **FR28 revised accordingly in `prd.md` in this story**, per the propagation rule Story 8.6's upstream drift established.
