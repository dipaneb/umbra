...
- **Drag-and-drop.** **Image to Text** and **PDF** tools accept drag-and-drop of files/screenshots as an alternative to a file picker — never as a replacement for it, since every flow must stay keyboard-drivable. The PDF tool additionally accepts a **multi-file** drop, which is what a merge queue actually needs. **Corrected 2026-09-10 (Story 8.8):** this line read *"Bucket (OCR) and PDF tools"* and was written 2026-08-15 — five days **after** Story 6.1 shipped a deliberate picker-only decision for PDF — so it described an intent the code had already declined, and stayed false for three and a half weeks. Story 8.8 re-decided drag-and-drop on today's facts and adopted it, making the line true; "Bucket" is also corrected to "Image to Text" per Story 8.7's rename.
